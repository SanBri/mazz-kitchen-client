...
const Section = ({
  ...
  color && (classDefinition += ` section__solid--${color}`);

  let inline = {};
  minHeight && (inline.minHeight = `${minHeight}`);
  fixed && ((inline.position = `fixed`), (inline.top = `${topPosition}rem`));
  marginTop && (inline.marginTop = `${marginTop}`);
